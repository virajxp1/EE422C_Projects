...
collect_results()
{
	...
	do
        ...
        then
		    brief_result_line=$( $PYTHONEXEC "$main_dir/helper_files/score_extractor.py" "$tested_submission/test_log.txt" "$tested_submission" )
...
perform_test_on_this_submission()
{
    ...
    if [ "$TA_MODE" = true ]
    then
	mvn clean test -DargLine="-Xss1024M -Xms1024M -Xmx2048M" > test_log.txt 2>&1 || true
    else
    	mvn clean test -DargLine="-Xss1024M -Xms1024M -Xmx2048M" 2>&1 | tee test_log.txt || true
    fi
...
prepare()
{
    ...
    cp "../../files_for_students/five_letter_words.txt" "$grading_project"
